...
def get_intermediate_links(quality_page_url):
    logger.debug(f"Extracting intermediate links from: {quality_page_url}")
    r = safe_request(quality_page_url)
    if not r:
        logger.warning(f"❌ No response from intermediate page: {quality_page_url}")
        return []
    soup = BeautifulSoup(r.text, "html.parser")
    links = []
    for tag in soup.find_all(["a", "button"]):
        ...
        if href and label and href.startswith("http") and not any(
            ...
            logger.debug(f"Intermediate link found: {label} => {href}")
            links.append((label, href))
    logger.debug(f"Total intermediate links: {len(links)}")
    return links
